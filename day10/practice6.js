...
// 주의할 점 : 속성명은 저장할 배열 내 존재하는 객체들의 속성명과 동일하게 구성하여 추후에 같은 배열에서 식별.
 let member = { id : 아이디, password: 비밀번호, name: 이름};
 // ========================== [*] 만약에 입력받은 아이디가 배열내 존재하면 {경우의 수 발생 }
 let find = false // 중복여부 체크 변수, true 중복 false 중복 아님. 스위치 변수 = on/off = 참/거짓 = 맞다/틀리다
 for( let index = 0; index <= members.length-1 ; index++){// 배열 내 모든 객체를 하나씩 조회 == 배열 순회 (0부터 마지막 인덱스 )
    let m= members[index]; // index번째 회원정보 (객체) 꺼내기 
    if( m.id == member.id ) { // 만약에 index  번째 회원정보의 아이디가 입력받은 아이디와 같으면
        //console.log("중복입니다."); // 불가능, 아직 뒤에있는 인덱스까지 확인 안했으므로 최종 결론 불가능.
        // 최종 결론 내지 말고 중복을 찾았다 못 찾았다 기록, 결론은 반복문 끝나고 하자.
        find = true; //찾았다고 기록하고
        break; // 반복문 탈출
    } 
} // for end
if(find == false){ // 중복이 없으면 
    members.push(member);// [3] 배열 내 생성한 객체 저장, 배열 변수명.push(새로운 객체);
...
for( let index = 0; index <= scores.length-1; index++){
    sum3 += scores[index].math; // index번째의 수학점수를 누적합계 하기
}// for end
console.log( sum3 / scores.length ); // 총합계/ 배열길이 == 요소 총 개수 == 객체 총 개수 == 학생총인원
...
for(let index = 0; index <= products.length-1 ; index++){
    if(products[index].id == [3]){ // 만약에 index번쨰 상품정보(객체) 내 id가 3이면
...
/* 주의할 점: 왼쪽은 문제 2번 풀이 방식, 오른쪽은 문제 4번 풀이 방식. 결과는 같음(편한 거 사용)
    let ary = [3, 4, 5];                            let ary = [3,4,5];
    let value = ary[0];                             let result = art[0] + 10;     
...
for( let index = 0; index <= users.length-1 ; index++){// [2] 배열/목록에서 isActive가 true인 사용자들 찾기 == 배열/목록 순회
    if( users[index].isActive == true){ // [3] 만약에 index번째 객체 내 isActive가 true이면
        activeUsers.push( users[index]); // 새로운 배열에 index번째 객체 추가
    }// if end
}// for end
...
for( let index = 0; index <= movies.length-1 ; index++){ // [2] 배열/목록에서 순회하여 각 title 조회하기
    movieTitles.push( movies[index].title); // index번째 객체 내 title 속성값을 새로운 배열에 추가(push)
} // for end
