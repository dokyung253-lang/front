...
console.log(person); // [2] 객체 전체 호출 : (객체 갖는)변수명
...
let values = Object.values( book ); // [1] 객체 내 값 변환: Object.value( 변수명 );
for ( let index = 0 ; index <= values.length -1 ; index ++){ // [2] 0부터 마지막 인덱스까지 배열순회(반복)
...
person.email= "qweq@we.com"; // [1] 객체 내 속성 추가: (객체)변수명.새로운속성명 = 값;
delete person2.age;          // [2] delete (객체)변수명.삭제할 속성명;  
...
const total = product.price * product.stock ; // [1] 객체 내 1개 속성값 호출: (객체)변수명.속성명
console.log(total);
...
user["job-title"]      
//  user.job-title;     // [1] [X] 불가능
console.log( user["job-title"]) // [2] 속성명에 기호 있으면, 접근 연산자(.)가 아닌 []대괄호 표기법 사용한다.
...
const character = {  name: '전사', hp: 200, mp: 50 }; // [1] "찾을속성명" in (객체)변수명: 존재하면 true 아니면 false
...
console.log( Object.keys(country)); // 객체 내 모든 키 반환
console.log( Object.values( country)); // 객체 내 모든 값 반환
...
// [1] key를 알면 value 알 수 있지만, value 이용한 key 알 수 없다. 즉, key(속성명) 중복값이 없고, value(값)은 중복 허용
...
for(let index = 0; index <= keys.length-1 ; index++){ //[2] 모든 key를 반복문으로 순회한다.
    ...
    if( inventory[keys[index]] < 10){                 // [3] 만약에 index번째 재고가 10보다 작으면/미만
        console.log(keys[index]);                      // [4] index번째 key값 출력하기
    } // if end
} // for end
...
for( let index = 0; index <= students.length -1; index++){ // [1] 배열순회 : 0부터 마지막 인덱스 까지
     const student = students[index]; // index번째 학생 객체 호출
...
// [3] data 안에 있는 요소들을 반복문으로 순회(0부터 마지막 인덱스까지) 하기
for( let index=0 ; index <= data.lenth -1 ; index++ ){ // for 반복 1번 할때마다 객체 1개 조회
  let row= data[index]; // index번째 '동' 정보(객체)

console.log(row); // index번째 '동' 정보(객체) 확인
console.log{ row.동별 , row.세대수, row["인구수(계)"], row["인구수(여)"]}

// [4] 객체 정보들을 HTMl 테이블로 표현하기, 표 = 배열 = <table>, 행 = 객= <tr>, 열 = 속성 = td
html += `<tr>
