...
// 2. 여러 개 객체들은 배열로 구성
// let memberList=[{ 'no' : 1 , 'id' : 'qwe', 'pw' : '1234' }, { 'no' : 2 , 'id' : 'qrty', 'pw' : '5678' },];

// [2] 함수 설계
// 1. 등록함수 : 매: x, 리: x, 처리: 입력받은 값을 객체 생성하여 배열에 저장
// 2. 로그인함수: 매: x, 리: x, 처리: 입력받은 값을 배열내 존재하는지 [비교] 

// [3] 함수 구현: 등록버튼을 클릭했을 떄
function signup( ){ console.log("hi");
    //1. 입력받은 값 가져온다.
    ...
    let memberList= localStorage.getItem("memberList"); // 로컬저장소 'memberList' 자료 가져오기  
    if(memberList == null){ // 자료가 비어있으면
       memberList =[]; // 새로운 배열 선언
    }else{ // 자료가 있으면
        memberList = JSON.parse( memberList ); // memberList타입변환
    }   console.log("hello");
    /************ ======================================= *************/
...
function login( ){
    // 1. 입력받은 값 가져온다.
    const signIdDom = document.querySelector(".loginId");
    const id = signIdDom.ariaValueMax;
    const signPwDom = document.querySelector(".loginPw");
    const pw = signPwDom.value;
    ...
    for( let index =0; index<= memberList.length-1; index++){
        ...
        if( member.id== id && member.pw == pw){ // index번째 멤버(객체)꺼내기  
