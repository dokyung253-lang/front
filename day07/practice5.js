...
/* let max = number[0]; // 첫번째 값을 가장 최댓값으로 임의로 지정/선언
if( max < number[1] ) max = number[1]; // 만약에 다음 값이 최댓값보다 크면 교체 / 수정
...
*****           1                       1 2 3 4 5           1~5     , 전체 줄 수(5) - 현재 줄 수(1) +1
****            2                       1 2 3 4             1~4     , 전체 줄 수(4) - 현재 줄 수(2) +1
***             3                       1 2 3               1~3     , 전체 줄 수(3) - 현재 줄 수(3) +1
**              4                       1 2                 1~2     , 전체 줄 수(2) - 현재 줄 수(4) +1
*               5                       1                   1       , 전체 줄 수(1) - 현재 줄 수(5) +1
                line은 1부터 까지 1씩 증가한다. for ( let line = 1; line <= 5 ; line++ ){ }
                star는 1부터( 5 4 3 2 1 ) 
*/

let output = ``; // 출력할 문자열들을 (누적) 저장하는 변수
for (let line = 1; line <= 5; line++) {  // 1부터 5이하)까지 1씩 증가한다. (5회)
    for (let star = 1; star <= 5 - line + 1; star++) {
        output += "*"; // 줄바꿈
    } // for end
    output += "\n"; // 줄바꿈
}
console.log(output); // 예상 출력:  \n\n\n\n\n
...
for (let index = 0; index <= userNames.length - 1; index++) {// (1) 배열 내 모든 이름 하나씩 꺼내기
    let name = userNames[index]; // (2) index 번쨰 이름 꺼내기
...
for (let index = 0; index <= seatLayout.length - 1; index++) {
    ...
    // row값 예상 : ['A1', 'A2', 'A3'] 또는 ['B1', 'B2', 'B3'], ['C1', 'C2', 'C3']
    for (let col = 0; col <= row.length - 1; col++) {
        console.log(row[col]); // 행 마다 col번째 열 추출 / 꺼내기
    } // for end
} // for end
...
for (let index = 0; index <= numbers2.length - 1; index++) {
    ...
    if (newAry.indexOf(num) == -1) { // .index( 찾을값 ) : 찾았으면 인덱스 변환 함수 , 없으면 -1 
        newAry.push(num);              // .push( 추가할 값 ): 배열 내 맨 마지막 요소 값 추가
    } // if end
} // for end
console.log(newAry);
...
let numbers3 = [5, 3, 4, 1, 2]; // 오름차순은 더 큰 값이 뒤로 이동한다. 스왑: let temp = a;  a=b; b = temp; (a를 temp에 넣고, b를 a에 넣는다.)
/* (1) 5 > 3일 시, 스왑 (2) 앞뒤의 인덱스끼리 비교 
1번째 비교. (1) 5 > 3 (2) 3 > 4 (3) 4 > 1 (4) 1 > 2 
            j > j + 1      j > j + 1
2번쨰 비교. (1) 5 > 3 (2) 3 > 4 (3) 4 > 1
3번쨰 비교. (1) 5 > 3 (2) 3 > 4
4번쨰 비교. (1) 5 > 3 */
...
for (let i = 0; i <= numbers3.length - 1; i++) {
    ...
    for (let j = 0; j < numbers3.length - 1; j++) {
        ...
        if (numbers3[j] > numbers3[j + 1]) { // 만약에 j값이 인접한 J+1보다 더 크면 스왑}
            let temp = numbers3[j + 1]
...
// 누적합계/누계 추가: 변수 += 새로운 값, 변수 = 변수 + 새로운 값
let html = ``; //빈 변수 선언
...
(2). 구현 조건                                                          (3). 출력 예시 (HTML)
for 반복문을 사용하여 6개의 좌석을 모두 출력합니다.                             빈좌석 예약석 
각 좌석은 하나의 <div> 태그로 표시합니다.                                      예약석 빈좌석
좌석 상태에 따라 <div> 태그에 CSS 색상을 다르게 적용합니다.                     예약석 빈좌석
...
let html2 = ` ` ;
for(let index = 0 ; index <= seatStatus.length -1 ; index++){ // 배열 순회: 배열내 인덱스 0부터 마지막 인덱스까지
    html2 += `<span style= "color : ${ seatStatus[index] == "빈좌석" ? "blue" : "red "} ;" > 
            ${seatStatus[index]} 
            </span> 
            `;
    if( (index + 1)% 2 == 0 ){ // 인덱스가 짝수(index 나누기 2의 나머지는 0)이면 줄바꿈처리 
        html2 += `<br />`;
    }
    } // for end
...
for( let index = 0; index <= carNumbers.length - 1 ; index++ ){
    ...
    if( min <= 30){ money = 1000; }
        ...
        money = parseInt((min - 30)/10) * 500 + 1000; // 10분 단위이므로 일의 자리수 제거 + 10분마다 500원 추가 + 기본요금 1,000원
        }    
