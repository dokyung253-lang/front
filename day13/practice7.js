...
// 1. 매개변수 : age
// 2. 리턴값: true/ false
// 3. 처리 : age가 20세 이상이면 true, 미만이면 false
...
console.log(result1)
console.log( isAdult(17) ); // 1. 함수 실행 > 2. 함수반환 > 3. 출력
...
function sumArray( array ){ // 매개변수란? 함수 호출 시 전달되는 인자값 연결되는 변수
  // 주의할 점: 매개변수도 지역변수 특징으로 함수밖 numebers를 인자로 전달하더라도 매개변수랑 다르다. (*배열과 numbers는 별개이다)
  let sum = 0 // 지역변수
...
function findLongestWord( array ){
  let longWord = words [0]; // 첫번쨰 인덱스 값을 가장 큰 단어로 가정하고 시작
...
let userScore = 0; // 전역변수란? js 파일 내 어떠한 { }안에서 선언되지 않은 변수
function gainScore(){ userScore += 10; } // 전역변수는 아무런 조건없이 함수 안에서 사용가능 vs 전역변수를 인수로 매개변수에게 전달
...
function findTopStudent(){
  let top = students[0]; // 첫번쨰 학생을 최고 점수 가정 시작
  for( let index= 1; index <= students.length-1 ; index++){
     if ( top.score < students[index].score){ // 만약에 최고 점수의 점수보다 index번쨰 객체의 점수가 더 크면 < 속성값 비교>
     top = students[index]; // index 번쨰 객체를 top에 저장                                               < 객체 대입 >
    }
  }// for end
return top.name; //최고점수를 객체내 점수가 아닌 이름 속성값 반환
} // f end
...
function renderProducts(){
  let html = ''; // 구성한 html 문자열을 저장하는 변수
  for (let index = 0; index <= products.length-1 ; index++){
    let prod = products[index]; // index번쨰 제품(객체) 호출
     html += `<div>
     <h4> ${ prod.name }</h4>
     <p>${ prod.price }</p>
     <p>${prod.stock}</p>
...
/*실습 7: 간단한 계산기
num1, num2, operator ( '+', '-') 세 개의 매개변수를 받는 calculator 함수를 만드시오. operator가 '+'이면 두 수의 합을, '-'이면 두 수의 차를 콘솔에 출력하시오. (if문 사용)*/
...
function renderList(){
  ...
  for( let index = 0; index <= todoList.length-1; index++){
    let todo = todoList[index]; // index번째 문자열 1개 호출해서 변수 담았다.
...
/*      
      변수의 종류 분류 시점: 선언
       - 선언 ㅣ let/ const / function등 키워드가 작성된 위치
