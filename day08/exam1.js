...
// sum = sum + score[0]; // 기존변수 = 기존변수 + 새로운 값     vs       기존 변수  += 새로운 값     < 누적 합계 >
// sum = sum + score[1]; 
// sum = sum + score[2];
// sum = sum + score[3];
// sum = sum + score[4];
...
// 반복되지 않는 코드 패턴 : (1) x (2) x (3) 인덱스가 0부터 마지막 인덱스까지 = 0부터 배열. length-1까지 1씩 증가
...
console.log( `총점 : ${ sum }, 평균 : ${ sum / 5 }`);  // 5는 scores.length
...
// let (scores2[0] == 100 );console.log( scores2[0] )
// let (scores2[1] == 100 );console.log( scores2[1] )
// let (scores2[2] == 100 );console.log( scores2[2] )
// let (scores2[3] == 100 );console.log( scores2[3] )
// let (scores2[4] == 100 );console.log( scores2[4] )
...
(1). 초기 데이터
학생 이름과 점수는 두 배열의 동일한 인덱스를 사용합니다. let nameArray = ['유재석', '강호동', '신동엽']; let scores = [92, 86, 72];
(2). 구현 조건
...
/*document.write("") : js 에서 html로 출력하는 함수 */

// (1) 이름 출력
let html = `` ; // html 마크업과 데이터들을 저장할 변수
for( let index = 0 ; index <= nameArray.length -1 ; index++ ){
    html += ( nameArray[ index ]); // 0부터 마지막 인덱스까지 1씩 증가 출력한다.
    // console.log( scores4 [ index ] ); // (3) : (1) (2) 합치기
    let b= parseInt( scores4 [index] / 10 );  //(4) 점수의 몫 구하기. 몫만큼 검은 별, paseInt(값 / 수)
    for( let i = 1; i <= 10; i++ ) { // 몫 만큼 검은 동그라미 출력하기
        if( i <= b ){html += ( "●" );}    // ㅁ + 한자 키
        else{ html += ( "○" ); }    
    } // for end
    // 학생이 바뀌면 줄바꿈처리 console : \n, HTML : <br />
} // for end
document.write( html );